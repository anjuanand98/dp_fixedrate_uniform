...
fun=@(xv) 1/(b-a);
for i=1:length(p)
    p(i)=integral(fun,xval(i)-delta/2,xval(i)+delta/2,'ArrayValued',true);
end
for N=Nval%number of quantization levels
[Xnopt]=nonstrategic_quantization_hloop(xval,N,p);
Yn=decoder(Xnopt,xval,p);
%encoder distortion
...
save('dataxcubeduniformfixed1.mat','Nval','Xnopt2','Yn2','endistortion2','dedistortion2','Xnopt4','Yn4','endistortion4','dedistortion4','Xnopt8','Yn8','endistortion8','dedistortion8','Xnopt16','Yn16','endistortion16','dedistortion16','Xnopt32','Yn32','endistortion32','dedistortion32');
...
title('fixed rate xcubed uniform','FontSize',14);
...
for i=1:size(D1val,1)%iterate over possible (alpha,beta) pairs
    ...
    D1(i)=encoderdistortion([D1val(i,1);D1val(i,2)],q,xval,p);
end
